...
### Resume on Re-payment

If an existing instance is in the `suspended` state (suspended by the overdue cron), paying the invoice triggers `action_resume()` instead of creating a new instance:

```python
if existing and existing.state == "suspended":
    existing.action_resume()  # Scales deployment back to 1
...
When `odoo_k8s_saas_subscription` is installed, the payment trigger also:

1. **Finds the subscription** linked to the sale order
2. **Sets `subscription_id`** on the instance at create-time (critical for avoiding duplicates)
3. **Flushes to DB** immediately with `self.env.flush_all()` to prevent race conditions
4. **Advances the subscription stage** to "In Progress" via `_link_subscription()`

```python
# Set subscription_id at create-time so the ORM flushes it
# to the DB before the stage-advance write() fires the
# SaleSubscription.write() hook.
...
# Flush instance to DB NOW
self.env.flush_all()

# Link subscription + advance stage
self._link_subscription(order, instance)
```

> **Important:** The `flush_all()` call is critical. Without it, `SaleSubscription.write()` can't find the instance via `search(subscription_id=…)` because the ORM hasn't flushed it to the DB yet — resulting in duplicate namespaces.
...
The sales integration adds these fields to `saas.instance`:

| Field | Type | Notes |
|:---|:---|:---|
| `sale_order_id` | `Many2one` | `sale.order` — the originating sale order |
| `odoo_version` | `Selection` | Copied from product's `odoo_version` field (default `18.0`) |
| `custom_image` | `Char` | Copied from product's `custom_image` field |

These fields are set by `_saas_check_and_provision()` and link the instance back to its sale order and product configuration.
...
> **For subscription-based SaaS plans** with recurring monthly invoicing, see **[Subscription Integration](Subscription-Integration.md)**. The one-time payment trigger documented on this page still works alongside subscriptions as a belt-and-suspenders approach — both triggers are idempotent.
