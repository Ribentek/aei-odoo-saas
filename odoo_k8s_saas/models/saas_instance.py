...
class SaasInstance(models.Model):
    ...
    def action_provision(self):
        ...
        try:
            ...
            # Log the per-instance support-user credentials as an INTERNAL note
            # (mail.mt_note — never emailed to the customer). odoo-init creates
            # this user on the tenant's first boot; the password is generated by
            # the portal and only returned here, so the chatter is its one
            # persistent record on the admin side.
            if data.get("support_password"):
                rec = self if self.env.uid else self.with_user(SUPERUSER_ID)
                rec.message_post(
                    body=_(
                        "Support user for this instance — login: %(login)s / password: %(password)s",
                        login=data.get("support_login") or "soporte@aeisoftware.com",
                        password=data["support_password"],
                    ),
